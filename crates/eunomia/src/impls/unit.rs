...
mod tests {
    ...
    fn division_matches_native<T: FloatElement + UnitScalar>() {
        for value in [T::ZERO, T::ONE, T::from_f64(2.0)] {
            assert_eq!(value.divide_by_factor(2.0), value / T::from_f64(2.0));
        }
    }
    ...
    fn complex_unit_scaling_preserves_quadrature() {
        ...
        let scaled = value.scale_by_factor(0.5);
    ...
    fn real_unit_division_avoids_reciprocal_overflow() {
        ...
        assert_eq!(
            coefficient
                .divide_by_factor(f64::from(coefficient))
                .to_bits(),
            1.0_f32.to_bits()
        );
        assert_eq!(
            0.0_f32.divide_by_factor(f64::from(coefficient)).to_bits(),
            0.0_f32.to_bits()
        );

        let coefficient = 1.0e-315_f64;
        assert_eq!(
            coefficient.divide_by_factor(coefficient).to_bits(),
            1.0_f64.to_bits()
        );
        assert_eq!(
            0.0_f64.divide_by_factor(coefficient).to_bits(),
            0.0_f64.to_bits()
        );
    }
    ...
    fn complex_unit_division_is_componentwise() {
        let coefficient = 1.0e-40_f32;
        let quotient =
            Complex32::new(coefficient, -coefficient).divide_by_factor(f64::from(coefficient));
        assert_eq!(quotient.re.to_bits(), 1.0_f32.to_bits());
        assert_eq!(quotient.im.to_bits(), (-1.0_f32).to_bits());
        let zero = Complex32::new(0.0, 0.0).divide_by_factor(f64::from(coefficient));
        assert_eq!(zero.re.to_bits(), 0.0_f32.to_bits());
        assert_eq!(zero.im.to_bits(), 0.0_f32.to_bits());

        let coefficient = 1.0e-315_f64;
        let quotient = Complex64::new(coefficient, -coefficient).divide_by_factor(coefficient);
        assert_eq!(quotient.re.to_bits(), 1.0_f64.to_bits());
        assert_eq!(quotient.im.to_bits(), (-1.0_f64).to_bits());
        let zero = Complex64::new(0.0, 0.0).divide_by_factor(coefficient);
